...
extern "C" {
  ...
  Sass_Data_Context* ADDCALL sass_make_data_context(char* source_string, bool should_clone)
  {
    ...
    try {
      ...
      ctx->source_string = should_clone ? sass_strdup(source_string) : source_string;
    } catch (...) {
      handle_errors(ctx);
    }
    return ctx;
